"""Import MTGDecks article JSON into the RAG store."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Path to downloaded MTGDecks article JSON.",
    )
...
def main() -> None:
    ...
    logger.info("Loading MTGDecks articles", extra=log_extra(path=str(args.path)))
    documents = load_strategy_article_index_file(args.path)
    logger.info("Prepared MTGDecks RAG documents", extra=log_extra(document_count=len(documents)))
    if args.dry_run:
        log_document_samples(logger, documents, sample_size=args.sample_size)
        logger.info(
            "Dry run completed; no MTGDecks RAG documents were imported",
    ...
    logger.info("Imported MTGDecks RAG documents", extra=log_extra(imported=count, path=str(args.path)))

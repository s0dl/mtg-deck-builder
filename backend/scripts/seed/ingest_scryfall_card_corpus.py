"""Import Scryfall card corpus JSON into the RAG store."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Path to downloaded Scryfall default-card JSON.",
    )
...
def main() -> None:
    ...
    logger.info("Loading Scryfall card corpus", extra=log_extra(path=str(args.path)))
    documents = load_scryfall_card_corpus_file(args.path)
    logger.info("Prepared Scryfall card corpus RAG documents", extra=log_extra(document_count=len(documents)))
    if args.dry_run:
        log_document_samples(logger, documents, sample_size=args.sample_size)
        logger.info(
            "Dry run completed; no Scryfall card corpus RAG documents were imported",
    ...
    logger.info("Imported Scryfall card corpus RAG documents", extra=log_extra(imported=count, path=str(args.path)))
